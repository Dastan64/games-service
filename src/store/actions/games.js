...
export const getInitialGames = () => {
    return (dispatch) => {
        fetch(`${process.env.REACT_APP_BASE_URL}games?page_size=30&dates=2021-01-01,2022-12-31&ordering=-added&key=${process.env.REACT_APP_API_KEY}`).then(response => response.json()).then(data => dispatch({
            type: LOAD_GAMES,
